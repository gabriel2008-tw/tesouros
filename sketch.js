...
var imgCaminho,imgMenino,imgDinheiro,imgDiamantes,imgJoias,imgEspada,G1, G2;
...
menino.scale=0.08;
menino.debug = false
menino.setCollider("circle",0,0,500)

G1 = createSprite(150,300)
 G1.visible = false
  G1.addAnimation("GameOver", G2)
   G1.scale = 0.5
...
function draw() {
  ...
  if(caminho.y > 400 ){
    ...
    if (dinheiroG.isTouching(menino)) {
      ...
      joiasG.destroyEach();
      colecaoTesouros=colecaoTesouros+75;
...
function criarDinheiro() {
  ...
  dinheiro.debug = false
  dinheiro.setCollider("circle", 0,0,200)
  }
}
...
function criarDiamantes() {
  ...
  diamantes.lifetime = 210;
  diamantesG.add(diamantes);
  diamantes.debug = false
}
}
